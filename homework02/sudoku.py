...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    s = []
    for i in range(3):
        s += grid[pos[0] - pos[0] % 3 + i][pos[1] - pos[1] % 3 : pos[1] - pos[1] % 3 + 3]
    return s
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    for i in range(1, 10):
        if (
            str(i) not in get_row(grid, pos)
            and str(i) not in get_col(grid, pos)
            and str(i) not in get_block(grid, pos)
        ):
            f.add(str(i))
    return f
